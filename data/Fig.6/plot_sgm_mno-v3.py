...
others_annotate = (0.75, 0.06)
# right_annotate = (0.68, 0.80)
EandG_annotate = (0.05, 0.06)

# 读取Giw数据
w_norg_t2g_giw, norg_t2g_re_giw, norg_t2g_im_giw    = numpy.loadtxt("./mno_giw/norg-mno-t2g.data", unpack=True, usecols=(0,1,2))
w_norg_eg_giw, norg_eg_re_giw, norg_eg_im_giw       = numpy.loadtxt("./mno_giw/norg-mno-eg.data", unpack=True, usecols=(0,1,2))
w_ctqmc_t2g_giw, ctqmc_t2g_re_giw, ctqmc_t2g_im_giw = numpy.loadtxt("./mno_giw/ctqmc-mno-t2g.data", unpack=True, usecols=(0,1,2))
w_ctqmc_eg_giw, ctqmc_eg_re_giw, ctqmc_eg_im_giw    = numpy.loadtxt("./mno_giw/ctqmc-mno-eg.data", unpack=True, usecols=(0,1,2))

# 子图 (a)
axs[0].annotate(r'(a) $t_{2g}$', others_annotate, xycoords='axes fraction', fontsize=12)
...
axs[0].set_ylim(-0.25, 0.0)
axs[0].set_yticks([-0.2, -0.1, 0.0])
# axs[0].tick_params(length=4, width=0.5, direction='in', top=False, right=False)


# 子图 (e)
axs[4].annotate(r'(e) $e_{g}$', others_annotate, xycoords='axes fraction', fontsize=12)
...
# 读取Siw数据
w_norg_t2g_siw, norg_t2g_re_siw, norg_t2g_im_siw    = numpy.loadtxt("./mno_siw/norg/t2g/t2g.data", unpack=True, usecols=(0,1,2))
w_norg_eg_siw, norg_eg_re_siw, norg_eg_im_siw       = numpy.loadtxt("./mno_siw/norg/eg/eg.data", unpack=True, usecols=(0,1,2))
w_ctqmc_t2g_siw, ctqmc_t2g_re_siw, ctqmc_t2g_im_siw = numpy.loadtxt("./mno_siw/ctqmc/t2g/t2g.data", unpack=True, usecols=(0,1,2))
w_ctqmc_eg_siw, ctqmc_eg_re_siw, ctqmc_eg_im_siw    = numpy.loadtxt("./mno_siw/ctqmc/eg/eg.data", unpack=True, usecols=(0,1,2))

# 子图 (b)
axs[1].annotate(r'(b) $t_{2g}$', others_annotate, xycoords='axes fraction', fontsize=12)
...
# 子图 (f)
axs[5].annotate(r'(f) $e_{g}$', others_annotate, xycoords='axes fraction', fontsize=12)
...
axs[5].set_xlim(0, 8)
axs[5].set_ylim(-5, 0.0)
axs[5].set_xticks([0, 4, 8])
axs[5].set_yticks([0, -1, -2, -3, -4, -5])
axs[5].tick_params(length=4, width=0.5, direction='in', top=True, right=True)

# ----------------------------------------------------------------------------------------------------------------------------------
# 读取Sigma数据
w_norg_t2g_sigma, norg_t2g_re_sigma, norg_t2g_im_sigma    = numpy.loadtxt("./mno-sigma/norg-t2g-sigma.data", unpack = True, usecols = (0,1,2))
w_norg_eg_sigma, norg_eg_re_sigma, norg_eg_im_sigma       = numpy.loadtxt("./mno-sigma/norg-eg-sigma.data", unpack = True, usecols = (0,1,2))
w_ctqmc_t2g_sigma, ctqmc_t2g_re_sigma, ctqmc_t2g_im_sigma = numpy.loadtxt("./mno-sigma/ctqmc-t2g-sigma.data", unpack = True, usecols = (0,1,2))
w_ctqmc_eg_sigma, ctqmc_eg_re_sigma, ctqmc_eg_im_sigma    = numpy.loadtxt("./mno-sigma/ctqmc-eg-sigma.data", unpack = True, usecols = (0,1,2))

# 子图 (c)
axs[2].annotate(r'(c) $t_{2g}$', others_annotate, xycoords='axes fraction', fontsize=12)
lines = axs[2].plot(w_norg_t2g_sigma, norg_t2g_re_sigma, w_ctqmc_t2g_sigma, ctqmc_t2g_re_sigma, alpha=0.8, clip_on=True)
axs[2].plot([0, 0], [500, -500], 'k--')
plt.setp(lines[0], linewidth=2.0, label='NORG')
plt.setp(lines[1], linewidth=2.0, label='CT-HYB')
axs[2].set_ylabel(r"Re$\Sigma(\omega)$", labelpad=3)
# axs[2].legend(frameon=False, loc='upper left', bbox_to_anchor=(-0.03, 1.03), ncol=1)
axs[2].set_xlim(-8, 4)
axs[2].set_xticks([-8, -4, 0, 4])
axs[2].set_xticklabels([])
axs[2].set_ylim(20, 50)
axs[2].set_yticks([30,40,50])
axs[2].tick_params(length=4, width=0.5, direction='in', top=False, right=False)


# 子图 (g)
axs[6].annotate(r'(g) $e_{g}$', others_annotate, xycoords='axes fraction', fontsize=12)
lines = axs[6].plot(w_norg_eg_sigma, norg_eg_re_sigma, w_ctqmc_eg_sigma, ctqmc_eg_re_sigma, alpha=0.8, clip_on=True)
axs[6].plot([0, 0], [500, -500], 'k--')
plt.setp(lines[0], linewidth=2.0, label='NORG')
plt.setp(lines[1], linewidth=2.0, label='CT-HYB')
axs[6].set_xlabel(r"$\omega$ (eV)", labelpad=-1.0)
axs[6].set_ylabel(r"Re$\Sigma(\omega)$", labelpad=3)
axs[6].set_xlim(-8, 4)
axs[6].set_xticks([-8, -4, 0, 4])
axs[6].set_ylim(20, 50)
axs[6].set_yticks([20,30,40,50])
axs[6].tick_params(length=4, width=0.5, direction='in', top=True, right=False)


# 子图 (d)
axs[3].annotate(r'(d) $t_{2g}$', others_annotate, xycoords='axes fraction', fontsize=12)
lines = axs[3].plot(w_norg_t2g_sigma, norg_t2g_im_sigma, w_ctqmc_t2g_sigma, ctqmc_t2g_im_sigma, alpha=0.8, clip_on=True)
axs[3].plot([0, 0], [500, -500], 'k--')
...
# 调整布局并保存图片
plt.tight_layout()
plt.savefig("mno-sgm"+present(), bbox_inches='tight', pad_inches=0.00625, transparent=True)
plt.savefig("../../tex-zen/mno-sgm", bbox_inches='tight', pad_inches=0.00625, transparent=True)